...
import { MAX_BLOCK_LEN, MAX_BLOCKS, MAX_WIRE_PAYLOAD_LEN } from "./limits.ts";
...
export function isValidHeader(h: FrameHeader): boolean {
  if (!Number.isInteger(h.sessionId) || h.sessionId <= 0 || h.sessionId > 0xffff) return false;
  if (!Number.isInteger(h.seq) || h.seq < 0 || h.seq > 0xffffffff) return false;
  if (!Number.isInteger(h.k) || h.k <= 0 || h.k > MAX_BLOCKS) return false;
  if (!Number.isInteger(h.blockLen) || h.blockLen <= 0 || h.blockLen > MAX_BLOCK_LEN) return false;
  if (!Number.isInteger(h.totalLen) || h.totalLen <= 0 || h.totalLen > MAX_WIRE_PAYLOAD_LEN) {
    return false;
  }
  if (!Number.isInteger(h.payloadFnv) || h.payloadFnv < 0 || h.payloadFnv > 0xffffffff) {
    return false;
  }

  const capacity = h.k * h.blockLen;
  const minimum = (h.k - 1) * h.blockLen + 1;
  return (
...
export function packFrame(h: FrameHeader, block: Uint8Array): Uint8Array {
  if (!isValidHeader(h) || block.length !== h.blockLen) {
    throw new RangeError("invalid optical frame header or block length");
  }
